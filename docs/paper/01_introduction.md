...
이러한 한계를 인식하고, 검색 기반 [Chauhan et al., 2022], 보간 기반 [GIMCC, 2025; TOI-VSF, 2024], 도메인 적응 [VIDA, 2025], end-to-end 복원-예측 통합 [GinAR, 2024] 등 VSF에 특화된 다양한 접근이 제안되었다. 이들은 의미 있는 성능 개선을 달성했으나, 결측 변수를 원시 시계열 공간에서 복원하거나 암묵적 어텐션에 의존하여, **(i)** 복원 오차의 예측 단계 전파, **(ii)** 복원 목적과 예측 목적 간 불일치, **(iii)** 원시 공간 복원 시 잠재 표현의 손실 중 하나 이상의 한계를 수반한다.

## P3: COMET 제안

본 논문은 이러한 한계를 근본적으로 해소하는 COMET (**CO**debook-augmented **M**ultivariate time-series forecasting with **E**xpertise **T**ransfer)을 제안한다. COMET은 결측 변수의 복원과 미래 값 예측을 임베딩 공간에서 단일 모델로 통합하는 end-to-end 프레임워크이다. 관측 변수를 패치 임베딩으로 인코딩하고, 결측 변수는 관측 변수 패치와의 cross-attention 및 codebook 패턴과의 cross-attention을 통해 임베딩 수준에서 2단계로 복원한 뒤, 복원된 임베딩으로부터 직접 예측을 생성한다. 원시 시계열 공간으로 되돌아가는 과정이 없으므로 앞서 지적한 세 가지 한계가 구조적으로 제거된다.
...
5개 실측 데이터셋에 대한 실험에서 COMET은 기존 방법 대비 SOTA 성능을 달성하며, 추론 시 단일 forward pass로 동작한다.
